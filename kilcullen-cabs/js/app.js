...
(function () {
  ...
  var T = window.KC_TARIFFS;
  var HOLIDAYS = window.KC_HOLIDAYS || [];
  ...
  function rateFor(dateStr, timeStr) {
    ...
    if (isNaN(d)) return "standard";
    var day = d.getDay(), h = d.getHours(), m = d.getMonth() + 1, dd = d.getDate();
    var special =
      ((day === 6 || day === 0) && h < 4) ||
      (m === 12 && dd === 24 && h >= 20) || (m === 12 && dd === 25) || (m === 12 && dd === 26 && h < 8) ||
      (m === 12 && dd === 31 && h >= 20) || (m === 1 && dd === 1 && h < 8);
    if (special) return "special";
    if (day === 0 || HOLIDAYS.indexOf(dateStr) !== -1 || h < 8 || h >= 20) return "premium";
    return "standard";
  }
  function fareFor(km, rateKey, passengers, isReturn) {
    var r = T[rateKey];
    var metered = r.initial + Math.min(Math.max(km - 0.5, 0), 14.5) * r.near + Math.max(km - 15, 0) * r.far;
    var trip = metered + T.bookingFee + T.extraAdult * Math.max((passengers || 1) - 1, 0);
    return isReturn ? trip * 2 : trip;
  }
  ...
  $$("[data-guide]").forEach(function (el) {
    var p = findPlace(el.dataset.guide);
    if (p) el.textContent = "~" + euro(fareFor(p.km, "standard", 1, false));
  });
  ...
  function trip() {
    var place = findPlace(f("b-dest").value);
    var pax = parseInt(f("b-pax").value, 10) || 1;
    var ret = f("b-return").checked;
    var rate = rateFor(f("b-date").value, f("b-time").value);
    return {
      place: place,
      rate: rate,
      pax: pax,
      ret: ret,
      fare: place ? fareFor(place.km, rate, pax, ret) : null,
    };
  }
  ...
  function renderFare() {
    ...
    if (t.place) {
      ...
      fareBox.innerHTML =
        '<p class="fare__amount"><small>~</small>' + euro(t.fare) + "</p>" +
        '<p class="fare__meta">' + T[t.rate].label + " · ~" + t.place.km + " km · ~" + t.place.min + " min" + (t.ret ? " · return" : " · one way") + "</p>" +
        '<p class="fare__note">Guide price from NTA maximum fares, incl. €3 booking fee. We confirm your exact fare before pickup.</p>';
    } else {
      fareBox.className = "fare fare--quote";
      fareBox.innerHTML =
        '<p class="fare__amount">Quote</p>' +
        '<p class="fare__meta">' + (dest ? "We'll price this trip when we reply" : "Add a destination to see a guide price") + "</p>" +
        '<p class="fare__note">Guide prices show for popular destinations like Dublin Airport, Naas and Newbridge. ' + T[t.rate].label + " applies at this time.</p>";
    }
  }
  ...
  function showDone() {
    ...
    rows.push(["Guide fare", t.place ? "~" + euro(t.fare) + " (" + T[t.rate].label.toLowerCase() + ")" : "Quote on reply"]);
    ...
    return '<p class="kicker sign-group">' + g[1] + "</p>" +
      PLACES.filter(function (p) { return p.sign === g[0]; }).map(function (p) {
        return '<article class="sign sign--' + p.sign + '" data-place="' + p.id + '">' +
          ...
          '<div class="sign__foot"><span>~' + p.min + " min · guide <b>" + euro(fareFor(p.km, "standard", 1, false)) + "</b></span>" +
